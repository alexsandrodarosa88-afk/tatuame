...
import { getMyPolicyStatus, acceptPolicy } from "@/lib/policy.functions";
...
export function PolicyAcceptGate() {
  ...
  const acceptFn = useServerFn(acceptPolicy);
  ...
  const mut = useMutation({
    mutationFn: () => acceptFn({ data: { userAgent: typeof navigator !== "undefined" ? navigator.userAgent : undefined } }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["policy-status"] }),
  });
    ...
    <div className="fixed inset-0 z-[100] bg-background/90 backdrop-blur-sm grid place-items-center p-4">
      <div className="w-full max-w-2xl rounded-2xl border border-border bg-card shadow-2xl flex flex-col max-h-[90vh]">
        <div className="p-5 border-b border-border flex items-center gap-2">
          ...
          <div>
            <h2 className="font-display text-lg font-bold leading-tight">Aceite das Políticas TATUAME</h2>
            <p className="text-xs text-muted-foreground">Versão {POLICY_VERSION} · necessário para continuar</p>
          </div>
        </div>
        ...
        <ScrollArea className="flex-1 min-h-0" ref={scrollRef as any}>
          <div className="p-5 space-y-4">
            <p className="text-xs text-muted-foreground">
              Como você já participou de uma campanha, pedimos seu aceite das nossas regras. Leia até o fim para liberar o aceite.
            </p>
        ...
        <div className="p-5 border-t border-border space-y-3">
          <label className="flex items-start gap-2 text-sm cursor-pointer">
            ...
            <span>
              Li e concordo com as Políticas TATUAME (v{POLICY_VERSION}), incluindo as regras de crédito (12 meses, intransferível, não resgatável em dinheiro, uso até 50% em tatuagens da rede TATUAME).
            </span>
          </label>
          {!reachedEnd && <p className="text-[11px] text-muted-foreground">Role o texto até o final para liberar o aceite.</p>}
          <div className="flex items-center justify-between gap-2">
            <Button asChild variant="ghost" size="sm"><Link to="/politicas">Abrir em página inteira</Link></Button>
            <Button
