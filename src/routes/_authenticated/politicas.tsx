...
import { getMyPolicyStatus, getMyArtistPolicyStatus } from "@/lib/policy.functions";
import { POLICY_SECTIONS, POLICY_VERSION, ARTIST_POLICY_SECTIONS, ARTIST_POLICY_VERSION } from "@/lib/policies";
...
function PoliticasPage() {
  ...
  const artistStatusFn = useServerFn(getMyArtistPolicyStatus);
  const { data: artistData } = useQuery({ queryKey: ["artist-policy-status"], queryFn: () => artistStatusFn() });
  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-3xl">
      ...
      {artistData?.hasAccess && (
        <div className="mt-12">
          <div className="flex items-center gap-2 mb-2 text-muted-foreground text-xs uppercase tracking-wider">
            <FileText className="h-4 w-4" /> Políticas do Tatuador · v{ARTIST_POLICY_VERSION}
          </div>
          <h2 className="font-display text-2xl font-bold mb-4">Políticas TATUAME — Tatuador</h2>
          {artistData.acceptedCurrent && artistData.latest && (
            <Card className="p-4 mb-6 border-primary/30 bg-primary/5 flex items-start gap-3">
              <CheckCircle2 className="h-5 w-5 text-primary mt-0.5" />
              <div className="text-sm">
                <div className="font-semibold">Você aceitou esta versão das políticas do tatuador.</div>
                <div className="text-muted-foreground text-xs">Versão {artistData.latest.version} aceita em {new Date(artistData.latest.accepted_at).toLocaleString("pt-BR")}.</div>
              </div>
            </Card>
          )}
          <div className="space-y-5">
